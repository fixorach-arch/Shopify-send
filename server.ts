...
async function startServer() {
  ...
  app.post('/api/send-campaign', async (req, res) => {
    ...
    try {
      ...
      // Verify account access (optional, but good for debugging)
      const accountApi = new SibApiV3Sdk.AccountApi();
      try {
        const account = await accountApi.getAccount();
        console.log('Brevo Account Name:', account.companyName);
      } catch (err: any) {
      ...
      if (!process.env.SENDER_EMAIL) {
        console.warn('WARNING: SENDER_EMAIL environment variable is not set. Using default sender email which may not be verified.');
      }
      ...
      const sendPromises = recipients.map(async (recipient: { email: string; name: string }) => {
        ...
        try {
          ...
          // Return error instead of throwing to allow other emails to proceed
  ...
  app.post('/api/send-test-email', async (req, res) => {
    ...
    try {
      ...
      console.log(`Configuration - Sender: ${senderName} <${senderEmail}>`);
      ...
      console.error('Test Email Error Details:', {
        ...
        body: error.response ? error.response.body : 'N/A',
        text: error.response ? error.response.text : 'N/A'
      });
